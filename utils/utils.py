...
sanitise_name = lambda name: re.sub(r'[:]', ' - ', re.sub(r'[\\/*?"<>|$]', '',
                                                          re.sub(r'[ \t]+$', '', str(name).rstrip()))) if name else ''
                ...
                try:
                    ...
                    if os.name == 'nt':
                        bar = tqdm(total=total, unit='B', unit_scale=True, unit_divisor=1024, initial=0, miniters=1,
                                   ncols=(columns - indent_level),
                                   bar_format=' ' * indent_level + '{l_bar}{bar}{r_bar}')
                    else:
                        raise
                except:
                    bar = tqdm(total=total, unit='B', unit_scale=True, unit_divisor=1024, initial=0, miniters=1,
...
def read_temporary_setting(settings_location, module, root_setting=None, setting=None, global_mode=False):
    ...
    if session and root_setting:
        if setting:
            return session[root_setting][setting] if root_setting in session and setting in session[
                root_setting] else None
        else:
...
def format_line(line: str, max_line_length: int = 30):
    # accumulated line length
    length = 0
    words = line.split(" ")
    formatted_line = ""
    for word in words:
        # if length + len(word) and a space is <= max_line_length
        if length + (len(word) + 1) <= max_line_length:
            # append the word and a space
            formatted_line = formatted_line + word + ' '
            # length = length + length of word + length of space
            length = length + len(word) + 1
        else:
            # append a line break, the word and a space
            formatted_line = formatted_line + '\n' + word + ' '
            # reset line length to the length of a word and a space
            length = len(word) + 1
